#this is a shortchut for commmands
#-include .env 

#if we want to make a shortcut for build then type,if you want to type your code in the same like use the :; as seen below,if not then use : then type your code on the next line
#build:; forge build
#to run on terminal type make build

#deploy-sepolia:	
#forge script script/DeployFundMe.s.sol:DeployFundMe --rpc-url $(SEPOLIA_RPC_URL) --private-key $(PRIVATE_KEY) --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv

#to run on terminal type make depoly-sepolia
